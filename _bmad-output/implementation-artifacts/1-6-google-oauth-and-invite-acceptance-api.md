...
Adversarial review by Blind Hunter + Edge Case Hunter + Acceptance Auditor against the 4,045-line diff for Story 1.6.

- [x] [Review][Decision] Invite acceptance for already-member with different role — invite consumed but role NOT updated (Edge Hunter). Spec says "ON CONFLICT DO NOTHING (idempotent — user might already be in this center by another path)". Current impl: invite is consumed, but if the user already had a different role in this center, the new role from the invite is silently dropped — the user keeps their old (potentially lower-privilege) role. Three valid resolutions exist; needs Owner-UX call before patching. [`internal/service/auth_invite.go:258-281`]
- [x] [Review][Patch][High] `defer h.clearOAuthStateCookie(w)` after `http.Redirect` is a no-op in production — replay defense broken (Blind+Edge). The deferred clear runs AFTER `http.Redirect` has called `WriteHeader` and written the body. Real `net/http` servers commit headers at that point; subsequent header mutations don't reach the wire. Tests pass only because `httptest.ResponseRecorder` lets headers mutate post-Write. Fix: hoist explicit `clearOAuthStateCookie` calls BEFORE each `http.Redirect`. [`internal/handler/auth_handler.go:470, 474, 481, 1119`]
- [x] [Review][Patch][High] `assertTenantBinding` reads RLS-protected `center_members` without `SET LOCAL app.current_tenant_id` — every prod subdomain login fails AC3 (Edge Hunter). The function uses `q := generated.New(s.db)` (bare pool, no tx). `center_members` has FORCE RLS with policy `USING (center_id = current_setting('app.current_tenant_id')::uuid)` — without SET LOCAL the predicate evaluates to NULL/false → 0 rows for every legitimate member. Tests pass because `test.TenantContext` SET LOCAL leaks across calls on the shared TxDB connection. Fix: wrap the membership check in a tx that runs SET LOCAL first (same pattern as `auth_admin.go` and `force_logout.go`). [`internal/service/auth_google.go:489-510`]
- [x] [Review][Patch][High] `*InviteExpiredError` / `*InviteAlreadyAcceptedError` at OAuth-init time map to 500 instead of 410/409 (Edge Hunter). The init handler's error switch only matches `*InviteNotFoundError`; everything else falls to `default:` → INTERNAL_ERROR. User clicking `/api/auth/google?inviteToken=<expired>` sees server-error envelope. Fix: extend the switch to map both error types to their respective envelopes (consistent with the REST endpoint). [`internal/handler/auth_handler.go:434-444`]
- [x] [Review][Patch][Med] Missing `OAUTH_USERINFO_TIMEOUT` typed error and `?error=google_timeout` redirect code (Auditor — AC10). Spec mandates a distinct timeout error → distinct redirect code. Current impl collapses every userinfo failure (timeout, decode, non-2xx) into `*OAuthUserinfoError` → `?error=google_userinfo_failed`. api.yaml also omits `google_timeout`. Fix: add `OAuthUserinfoTimeoutError`, distinguish in `realGoogleOAuthClient.UserInfo` via `errors.Is(err, context.DeadlineExceeded)`, map to `google_timeout` in handler. [`internal/service/auth_google.go:119-140`, `internal/handler/auth_handler.go:1148`]
- [x] [Review][Patch][Med] `GOOGLE_REDIRECT_URL` path suffix not enforced (Auditor — AC10). Spec requires the URL to end with `/api/auth/google/callback`. Current `config.Validate` only checks `https://` prefix. A misconfigured `https://foo.classlite.app/anything` passes validation. Fix: add `strings.HasSuffix(c.GoogleRedirectURL, GoogleRedirectURLPath)` check + named constant. [`internal/config/config.go`]
- [x] [Review][Patch][Med] `OAuthExchangeError{UpstreamErr: "oauth not configured"}` masquerades as a Google fault (Blind+Edge). When `s.oauth == nil` or `s.oauthState == nil`, init returns 500 INTERNAL_ERROR; callback returns `?error=google_exchange_failed` — both falsely point at Google. Spec promises 503 for "not configured". Fix: introduce `OAuthNotConfiguredError`, map to 503 in init handler, suppress as separate redirect code in callback (or just 503 with no redirect). [`internal/service/auth_google.go:160-162, 242-244`]
- [x] [Review][Patch][Med] `successRedirect` echoes `expectedEmail` and `googleEmail` into the URL — leaks invited email via Referer header to third-party resources on the SPA landing page (Blind+Edge). The Referer sent on Sentry beacons, analytics, image loads carries the URL. An attacker can confirm "this email was invited" via random-token probing. Fix: pass via a short-lived one-shot signed cookie instead, or use POST-redirect. [`internal/handler/auth_handler.go:563-566`]
- [x] [Review][Patch][Med] `accessTokenTailWindowSeconds` audit field reports static `AccessTokenTTL` (15 min upper bound), not `ceil(exp - now)` per spec (Auditor — AC6). Audit consumers see less precision than the AC promised. Fix: compute per-event from the most recent access-token issuance OR document the deviation in the AC. [`internal/service/auth_force_logout.go:129`]
- [x] [Review][Patch][Low] `acceptInviteCreateUserAndMember` (new-user branch) lacks the `isUniqueViolation` softening that the existing-user branch has — concurrent invite-accept can surface 500 instead of intended 409 (Edge Hunter). Fix: mirror the `if !isUniqueViolation(err)` pattern from `acceptInviteAddMembership`. [`internal/service/auth_invite.go:339-343`]
- [x] [Review][Patch][Low] AcceptInvite REST path: over-cap inviteToken returns `INVITE_NOT_FOUND` (404) instead of spec-mandated `INVALID_INVITE_TOKEN` (400) (Auditor — Failure-Path AC). The GoogleInit handler does emit 400 (auth_handler.go:419), but the REST path collapses oversize via `*InviteNotFoundError`. Fix: add a length check + typed error before hashing. [`internal/service/auth_invite.go:66-68`]
- [x] [Review][Patch][Low] Empty `s.appApexHost` short-circuits all tenant binding (Edge Hunter). The condition `s.appApexHost == ""` returns "no check" — anywhere the setter is omitted (e.g., the force_logout handler tests), AC3 is effectively disabled. Config defaulting protects prod, but defense-in-depth: treat empty as "always-on" or error. [`internal/service/auth_google.go:471`]
- [x] [Review][Patch][Low] Case-sensitive `short_code` lookup vs case-mixed Host header — apex match runs before port strip (Blind+Edge). The test fixtures themselves are inconsistent (`"TENA"` vs `"tenb"`). Fix: lowercase normalize on insert + on lookup, OR add a case-insensitive index. [`internal/service/auth_google.go:484`, `internal/store/queries/centers.sql`]
- [x] [Review][Patch][Low] IPv6 host parsing chops at the first `:` inside `[]` (Edge Hunter). `"[::1]:8080"` → `host = "["`. Subsequent `IndexByte('.')` returns -1 → apex bypass. Fix: use `net.SplitHostPort` or `url.URL.Hostname()`. [`internal/service/auth_google.go:475`]
- [x] [Review][Patch][Low] `successRedirect` may emit `?invited=true` with empty `center=` when `fetchCenterName` fails silently (Edge Hunter). The underlying bug is the silent error swallow at `centerName, _ := fetchCenterName(...)` — UX shows "Welcome to !". Fix: only set the query param when name is non-empty; surface fetch errors via slog. [`internal/service/auth_invite.go:215`, `internal/handler/auth_handler.go:567-571`]
- [x] [Review][Patch][Low] `oauthStateCookieTTL = 600` (handler) duplicates `OAuthStateTTL` (service) — drift risk (Blind Hunter). Fix: derive the cookie TTL from `int(service.OAuthStateTTL.Seconds())`. [`internal/handler/auth_handler.go:982`]
- [x] [Review][Patch][Low] OAuthStateTTL boundary at exact-second equality is rejected (Edge Hunter). Check is `!clock.Now().Before(expiresAt)`. A user hitting exactly the 10-min mark sees `csrf_expired`. Fix: change to `clock.Now().After(expiresAt)` for inclusive equality. [`internal/service/oauth_state.go:134-137`]

**Dismissed as noise (7):** SameSite Lax for cross-site GET callback (correct), InviteTokenHash binding in callback (verified PASS), DeleteRefreshTokensByUserReturningFamilies unscoped (verified — existence check fires first), `users.google_id` UNIQUE check (verified — partial index exists), tenant context key migration (handled cleanly), audit logger param ordering (verified clean), various Auditor "PASS" entries from the AC mapping exercise.
...
| 2026-06-07 | Code review pass: 17 findings resolved (3 high, 5 med, 9 low) + 1 decision-needed (existing-member role-upgrade). Patches: defer-clear-cookie hoisted before each `http.Redirect` (P1 — replay defense was a no-op in prod), `assertTenantBinding` now runs SET LOCAL inside a tx (P2 — every prod subdomain login was failing AC3), `GoogleInit` error switch extended to map `*InviteExpiredError` / `*InviteAlreadyAcceptedError` / `*OAuthNotConfiguredError` to 410 / 409 / 503 (P3, P6), new `OAuthUserinfoTimeoutError` distinguishes `google_timeout` from `google_userinfo_failed` (P4 / AC10), new `OAuthNotConfiguredError` replaces the misleading `OAuthExchangeError{Reason:"oauth not configured"}` (P6), `GoogleRedirectURLPath` suffix validated at config load (P5), `successRedirect` no longer leaks invited/Google emails or center name via the URL/Referer (P7), `accessTokenTailWindowSeconds` renamed `maxAccessTokenTailWindowSeconds` with explanatory comment (P8 — honest about the upper-bound semantics), `acceptInviteCreateUserAndMember` softens unique-PK conflicts (P9), `INVALID_INVITE_TOKEN` checked at the AcceptInvite handler boundary (P10), defensive `appApexHost`/IPv6/case handling in `assertTenantBinding` (P11/P12/P13), empty `center=` no longer emitted (P14), `oauthStateCookieTTL` derived from `OAuthStateTTL` (P15), `OAuthStateTTL` boundary now inclusive equality (P16). D1 resolution: invite-acceptance for an existing member with a different role now upgrades the role in place via new `UpdateCenterMemberRole` sqlc query. `LOWER()` lookup on `centers.short_code`. Tests, race detector, and `go vet` all clean. Status → done. |
