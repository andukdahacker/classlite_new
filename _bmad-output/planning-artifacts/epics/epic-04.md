...
- Given the AI generation worker implementation, When integration tests run, Then it uses `test.SetupWorkerHarness` from `classlite-api/internal/test/workers/harness.go` with the 3 mandatory adversarial patterns: HappyPath, PayloadCenterIdIgnored, NullTenantContextRejected. Job row's `center_id` is the sole tenant trust anchor — payload `center_id` (if present) is logged as discrepancy signal. (R3 / A7 mitigation.)
- Given AI generation credit consumption, When a job is enqueued, Then a `-1` ledger entry is inserted in `ai_credit_ledger` (defined in Story 6.5) in the SAME transaction as the job insert. On `failed` final state after 3 retries OR after the 5-min stuck timeout, a `+1` refund row is inserted in the same tx as the state transition with `reason='job_failed_refund'` and `ref_job_id` linking back. Idempotency: unique index `(ref_job_id, reason)` rejects duplicate refunds. (R23 / A6 mitigation.)
...
- Given the per-feature file size caps (locked A9), When the upload runs, Then the following maximums apply: **Knowledge Hub PDF 50 MB · Knowledge Hub image (PNG/JPG/SVG) 15 MB · Listening audio (MP3/WAV) 100 MB**. Exceeding the cap returns 413 with i18n error code `FILE_TOO_LARGE` and a message including the cap in MB.
- Given enforcement is defense-in-depth, When the upload flow runs, Then: (1) client pre-check rejects oversized files immediately (UX, not security); (2) server pre-check at `/uploads/presign` rejects with 413 `FILE_TOO_LARGE` BEFORE generating the presigned URL; (3) R2 presigned URL signs a `Content-Length-Range` matching the per-feature cap (R2 rejects upload server-side if exceeded); (4) `/uploads/confirm` HeadObject re-validates actual stored size — delete the object and return 413 if any layer was bypassed.
- Given a file is being uploaded, When the per-feature cap passes AND the cumulative plan storage is checked, Then: at 80% of plan storage a soft warning appears in Settings → Storage; at 95% a prominent banner + email notification is sent to the owner (Resend, Vietnamese-localized); at 100%+ new uploads are hard-blocked with the message "Storage full. Delete files or upgrade to Studio." Existing files remain accessible.
- Given the R2 presigned URL flow (locked A10 policy: expiry + content-type + prefix validation, no dedup table), When `/uploads/presign` runs, Then: (a) server-side `{center_id}` prefix validation matches the JWT `center_id` (mismatch → 403 `R2_KEY_PREFIX_MISMATCH` + audit log entry); (b) Content-Type is locked in the signed payload (PUT with wrong Content-Type fails at R2); (c) MIME allowlist validated server-side BEFORE presigning — never trust client claim; (d) expiry tightened to **5 minutes** (architecture said 5-15; locked to 5).
- Given the `/uploads/confirm` endpoint, When it runs, Then it: (a) re-validates `{center_id}` prefix against JWT; (b) reads R2 HeadObject metadata to verify actual stored Content-Type matches the locked one; (c) verifies object size ≤ per-feature cap; (d) verifies object exists. Any mismatch → 403 + delete object server-side.
- Given the slog configuration, When any code path could log a presigned URL or signature, Then slog filters out URLs matching `s3.amazonaws.com|r2.cloudflarestorage.com` AND `X-Amz-Signature` query params — never logged. CI step grep-asserts this in test-run logs.
- Given the abuse safeguard, When a single tenant uploads >100 GB cumulative within 24 hours, Then a background ops alert fires (Sentry warning, not user-facing) for ops review.
- Given the R2 replay-detection telemetry hook, When `/uploads/confirm` runs and HeadObject shows the object was already confirmed by a previous `/confirm` call for the same key, Then this event is counted in metrics. Policy: if >3 such events/month observed post-launch, promote presigned URL design to Option B (dedup table approach).
