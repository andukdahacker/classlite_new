...
> - **5-5b (SHIPPED 2026-08-19)** — the graded display below (overall band, per-criterion breakdown, feedback + inline anchored comments, late-penalty math), net epic ACs #1-5,7,8. **FRONTEND-ONLY**: 6.1 already shipped the entire backend grade surface onto the existing `GET /api/assignments/{id}/result` (`released:boolean` + `grade:StudentGradeView|null`), so 5-5b added no api.yaml/codegen/Go/migration. It renders the graded block ABOVE the read-back on the durable `/submission` shell when `result.released === true` (band-ring hero + four per-criterion bars with pinned counts + feedback quote + read-only anchored comments painted via 6.1's shared `buildEssayHtml` + neutral FR-31 penalty math), and registers `/assignments/:id/result` as a **redirect alias → `/submission`** (D-ROUTE — not a duplicate page). Writing is the only graded skill in v1; released speaking (6.3) / quiz (6.4) grades hard-gate to a "result coming soon" state (FU-5-5b-SPEAKING / FU-5-5b-QUIZ). A per-device client-tracked "new result" unread indicator was added to the `/assignments` list (D-DISCOVERY; durable cross-device → FU-5-5b-DISCOVERY). (ACs #6/#9 pending/404 were superseded by the 5-5a D1/D2 reframe.)
