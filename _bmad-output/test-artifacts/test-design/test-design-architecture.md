...
| **R23b** | BUS | Speaking `partial_success` (transcription failed, bands proposed) is a ratified NO-refund complete — a coding slip could refund it (revenue leak) OR a total failure could be laundered AS partial_success to dodge a refund the teacher is owed (silent overcharge). | 2 | 3 | 6 | Bands validated BEFORE the partial-return (bands-absent → terminal+refund, T-C); partial_success ±control ledger assertions (deduct only, no refund); an instrumented per-run `partial_success` counter/log so the ratified no-refund line is revisitable with data, not support tickets. Story 6-3b (D3/D15). | Backend lead | Epic 6 |
